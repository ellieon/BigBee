...
Please note that due to the nature of some content and subject matters regularly discussed, this is strictly an 18+ community, and we will take measures to remove members who break this rule
...
Have fun!!!`
  ...
  public init (): void {
    this.getClient().on('guildMemberAdd', (member) => {
      this.greeter(member)
    })
  }
  ...
  private async fallbackGreeter (member: GuildMember | PartialGuildMember) {
    const channel: TextChannel =
      member.guild.channels.cache
        .find((channel) => channel.name === EnvironmentHelper.getGreeterChannel()) as TextChannel
    if (channel) {
      channel
        .send(format(Greeter.GREETING_STRING, `<@!${member.id}>`, `<@!${this.getClient().user.id}>`))
        .catch(logger.error)
